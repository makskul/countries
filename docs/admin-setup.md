...
1. **Статья о стране** — Админка → «Страны и статьи» → карандаш → блок «Статья о стране» (UK / EN / RU). Кнопки **EN → UK / EN → RU** копируют черновик. Справа — превью «как на сайте».
2. **Статья о городе** — «Города» → карандаш → тот же редактор статьи.
3. **Отзывы** — фильтры: статус, демо/реальные, страна, национальность, поиск по городу. Карточка показывает оценки по-человечески, не JSON.
