-- Anon INSERT into reviews with city_id failed:
-- trg_refresh_city_stats runs as invoker and tries to upsert city_stats,
-- which only has a public SELECT policy → RLS 42501 and the whole review insert rolls back.
-- Make refresh SECURITY DEFINER and skip pending inserts (stats only care about approved).
